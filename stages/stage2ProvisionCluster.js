...
const stage2ProvisionCluster = async (formData, baseRepoPath) => {
  try {
    ...
    console.log('Running terraform workspace init...');
    ...
    broadcast({ stage: 2, message: 'Stage 2 complete: terraform.tfvars written and cluster provisioned' });

  } catch (err) {
    console.error('Stage 2 failed:', err.message);
    broadcast({ stage: 2, error: true, message: 'Stage 2 failed: ' + err.message });
    throw err;
  }
};
